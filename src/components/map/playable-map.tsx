...
import StakerRewards from '@/components/wallet/staker-rewards'
...
export default function PlayableMap() {
    ...
    const [activeView, setActiveView] = useState<'map' | 'stakes' | 'rewards'>('map');
  ...
  return (
    <div className="relative h-screen overflow-hidden">
      ...
      <div className='fixed bottom-0 left-0 w-full h-20 bg-black/90 backdrop-blur-lg border-t border-gray-700 z-10 flex items-center justify-around px-4 shadow-2xl shadow-black/40'>
        ...
        >
          <TrendingUp className={`w-6 h-6 mb-1 ${activeView === 'rewards' ? 'drop-shadow-lg filter drop-shadow-[0_0_8px_rgba(239,68,68,0.6)]' : ''}`} />
        </div>
        <div 
          className='relative flex flex-col items-center justify-center cursor-pointer hover:scale-110 active:scale-95 transition-all duration-300 p-2.5 rounded-2xl text-gray-300 hover:text-red-300 hover:bg-gradient-to-br hover:from-red-500/20 hover:to-red-600/10 hover:shadow-lg hover:shadow-red-500/30 hover:border hover:border-red-500/40'
